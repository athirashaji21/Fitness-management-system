...
if(isset($_POST['login1'])){
	...
	$query="SELECT trainer_pwd from trainer_login_details WHERE trainer_id='$id' ";
	$result=mysqli_query($conn,$query);
	$row=mysqli_fetch_array($result);
	if($row['trainer_pwd']==$pass)
	{
		session_start();
		$_SESSION['user_id']=$id;
		header("location:../front_end/trainer_home");
	}
	else
